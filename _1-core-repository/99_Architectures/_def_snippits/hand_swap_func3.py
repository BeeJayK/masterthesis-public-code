...
X = tf.convert_to_tensor(
    np.array([1, 6, 6, 6, 0, 7, 7, 7, .906, .92], dtype=np.float32)
)
...
skip_eval = tf.Variable(np.array([1 for _ in range(6)]), dtype=tf.float32)
# inputs from cpa
pos_0_idx = 0
pos_1_idx = 4
...
print(X)
X = tf.cond(
    tf.math.reduce_sum(skip_eval) > check_const_0,
    pass_func,
    swap
)
print(X)
...
# NOTE: due to datastructure changes, undetected hands are now signaled by
# 		values of -1. As the values are used across several layers, the easiest
# 		adjustment that is now done, is to simply check in the swapper-layer
# 		for -1 at the handedness result and insert in this case a nan, to easaly
# 		continue using all the logics.
# 		same goes for the handedness-pred-confidences (are they even used when
# 		the prediction itself is already nan? dont think so, which would mean,
# 		nothing to do there...)
# ---------------------
