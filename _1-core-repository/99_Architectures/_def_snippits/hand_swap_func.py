...
X = tf.convert_to_tensor(
    np.array([1, 6, 6, 6, np.nan, 7, 7, 7, .96, np.nan], dtype=np.float32)
)
...
# SKIP 1
ans = tf.cond(skip1(hs), direct_return, pass_cond)
dec_sto.append(ans)
print("skip1: ", ans)

# SKIP 2
ans = tf.cond(skip2(nan_count, check_const_2), direct_return, pass_cond)
dec_sto.append(ans)
print("skip2: ", ans)

# SKIP 3
ans = tf.cond(skip3(hs, nan_mask, check_const_1), direct_return, pass_cond)
dec_sto.append(ans)
print("skip3: ", ans)
...
print(X)
if "direct_return" not in dec_sto:
    X = swap(X)
print(X)
